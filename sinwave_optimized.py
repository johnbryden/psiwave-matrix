...
try:
    ...
    PERFORMANCE_MODES['numba'] = True
    print("🚀 Numba JIT compilation: ENABLED")
except ImportError:
    PERFORMANCE_MODES['numba'] = False
    print("⚠️  Numba JIT compilation: NOT AVAILABLE")

# Try to import Cython extensions
try:
    import cython_optimized
    PERFORMANCE_MODES['cython'] = True
    print("🚀 Cython extensions: ENABLED")
except ImportError:
    PERFORMANCE_MODES['cython'] = False
...
# Performance-optimized pixel blending functions
if PERFORMANCE_MODES['cython']:
    # Use Cython extensions (fastest)
    def _fast_pixel_blend(current_color, c1, c2, c3):
        return cython_optimized.fast_pixel_blend_cython(current_color, c1, c2, c3)
    
    def _fast_color_dim(color, dim_factor):
        return cython_optimized.fast_color_dim_cython(color, dim_factor)
    
    def _fast_sine_calc(x, amplitude, frequency, phase, vertical_offset):
        return cython_optimized.fast_sine_calc_cython(x, amplitude, frequency, phase, vertical_offset)
    
    print("   → Using Cython extensions (fastest)")
    
elif PERFORMANCE_MODES['numba']:
    ...
    print("   → Using Numba JIT compilation (fast)")
    
else:
    # Fallback to pure Python (slowest)
    def _fast_pixel_blend(current_color, c1, c2, c3):
        blended_r = max(current_color[0], c1)
        blended_g = max(current_color[1], c2)
        blended_b = max(current_color[2], c3)
    ...
    print("   → Using pure Python (slowest)")
...
def draw_sine_wave(
    ...
    for x in range(matrix.width):
        # Calculate the y-coordinate using optimized sine calculation
        y_center = round(_fast_sine_calc(x, amplitude, frequency, phase, vertical_offset))
        ...
        for w in range(1, width):
            ...
            # Pixel below center (dimmer)
            if y_center + w < matrix.height:
                draw_pixels(canvas, x, y_center + w, soft_colour[0], soft_colour[1], soft_colour[2], blend=blend)

# --- Main execution block ---
if __name__ == "__main__":
    print(f"\n🎯 Performance mode: {list(PERFORMANCE_MODES.keys()) if any(PERFORMANCE_MODES.values()) else 'Pure Python'}")
    ...
    print("Starting optimized sine wave animation... Press CTRL-C to stop.")
    try:
        while True:
            colour = [50, 50, 255]
            canvas.Clear()
